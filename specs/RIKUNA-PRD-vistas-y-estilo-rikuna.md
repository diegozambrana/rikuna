...
| Navegación principal | `NavigationMenu` o barra simple + `Avatar`/`DropdownMenu` para el menú de cuenta |
...
Organizado en tres zonas según quién puede entrar: **Auth** (sin sesión, flujo de acceso), **App** (requiere sesión), **Pública** (sin sesión, solo lectura de contenido compartido). Esta separación debe reflejarse en la estructura de rutas para que el middleware de autenticación no bloquee por error la zona Pública.
...
#### `/` o `/panel` — Panel principal ("Qué ver este mes")
- **Propósito:** vista de aterrizaje tras iniciar sesión.
- **Contenido:**
