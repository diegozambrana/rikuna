...
> Documento complementario en español. La fuente de verdad es `specs/backlog/RIK-11_public_list_sharing.md` — ante cualquier discrepancia, ese archivo manda. El prompt XML es idéntico en ambos documentos y no se traduce.

## Resumen del ticket

El MVP de Rikuna tiene exactamente una excepción deliberada a "todo requiere sesión": un usuario puede marcar una lista como pública y compartir un enlace que funciona para cualquiera, con o sin cuenta. Este ticket construye esa excepción de punta a punta — las páginas reales del grupo de rutas `(public)` (`/l/[codigo]` para una lista compartida, y la rama sin sesión de `/titulo/[slug]`), más la pieza de esquema que le falta: un `public_code` globalmente único en `user_lists`, porque el `slug` interno es único solo por usuario y no puede ser el identificador público de forma segura una vez que se abra el multiusuario (esquema, Sección 11.6).

- Agregar una nueva migración que le da a `user_lists` una columna `public_code`, generada una sola vez cuando la lista se hace pública por primera vez, y usarla (nunca `slug`) como identificador de `/l/[codigo]`.
- Implementar `getPublicListUrl`, que RIK-10 deja como placeholder para "copiar enlace".
- `/l/[codigo]` debe renderizar sin sesión alguna y mostrar solo el nombre, descripción y cuadrícula de títulos de esa lista — nada más de la cuenta del dueño.
- Marcar una lista como privada debe hacer que `/l/[codigo]` deje de resolver para todos excepto el dueño.
- Click en un título desde la lista pública debe llevar a la rama pública de `/titulo/[slug]` — la misma ruta que usan los usuarios autenticados (RIK-9), controlada por una bandera `isPublicView` en vez de una ruta duplicada, ya que los grupos de rutas de Next.js no pueden resolver la misma URL dos veces.
- No existen comentarios de equipo más allá de la propia nota del ticket, tratada como autoritativa: este ticket cierra el placeholder de RIK-10 y extiende el componente de RIK-9 en vez de construir ninguno de los dos desde cero.
...
**Nota:** `user_lists`, `list_items` y el grupo de rutas `(public)` todavía no existen en el proyecto — son responsabilidad de RIK-1 (esquema) y RIK-2 (estructura de rutas), ambos tickets hermanos en curso. El propio cambio de esquema de este ticket (la columna `public_code`) es un ALTER sobre la tabla de RIK-1, entregado como una migración nueva y con timestamp separado — nunca una edición al archivo de RIK-1.

### Comentarios del equipo

El propio texto del ticket trae una nota que define alcance (no existen comentarios de equipo separados para este ticket), tratada como autoritativa:

> This ticket both finishes RIK-10's "copiar enlace" placeholder (implementing `getPublicListUrl`) and adds the public-read branch to RIK-9's `/titulo/[slug]` component. Cross-check `RIKUNA-PRD-vistas-y-estilo-rikuna.md` Section 2.3 for the minimal public layout (no nav, just logo + login/signup CTA) and Section 3 ("radio de borde 0 en todos los componentes" — Lyra style constraint applies here too). This is the ticket most sensitive to the privacy rule in `RIKUNA-PRD-documento-especificacion-rikuna.md` Section 11 ("una lista es pública solo si el dueño lo marca explícitamente, y solo esa lista puntual queda visible") — the acceptance criteria and constraints must be airtight about not leaking owner account data.

Esto redirige el alcance de dos formas concretas:
1. `getPublicListUrl` **no** es una función de RIK-10 — RIK-10 solo conecta el botón "copiar enlace" en la UI; este ticket es dueño de la función misma, porque no puede existir antes de que exista `public_code`.
2. La rama pública de `/titulo/[slug]` es una **extensión** del componente de RIK-9 (vía una prop/bandera), no una segunda ruta — la ficha "pública" y la "autenticada" son la misma página física.
...
| --- | --- | --- |
| Las rutas viven "bajo `(public)`" | `app/` hoy solo tiene `layout.tsx`, `page.tsx`, `globals.css`, `favicon.ico` (scaffold de Create Next App) — no existen `(public)`, `(app)` ni `(auth)`; RIK-2 (dependencia) solo *reserva* el concepto en `ARCHITECTURE.md` | Este ticket crea el layout del grupo `(public)` y sus dos primeras páginas reales, no solo un agregado a un grupo existente. |
| El identificador público debe ser distinto de `user_lists.slug` | `user_lists` tampoco existe todavía en la base de datos — `supabase/migrations/` está vacío; el propio spec de backlog de RIK-1 (`specs/backlog/RIK-1_database_schema_rls.md`) confirma que creará `user_lists` exactamente según la Sección 6 del esquema, **sin** columna `public_code` | La migración de este ticket es un ALTER sobre una tabla que no existe hasta que RIK-1 aterrice — debe ser un archivo nuevo, con timestamp posterior, nunca una fusión con la migración de RIK-1. |
| `/titulo/[slug]` "variante pública... sin las acciones que requieren sesión" | La entrada `features/title` de `ARCHITECTURE.md` ya modela esto como **un** componente compartido "with an `isPublicView` flag gating the personal-action buttons" — y los grupos de rutas de Next.js prohíben explícitamente que dos carpetas resuelvan a la misma URL (`route-groups.md`: "Conflicting paths... would both resolve to `/about` and cause an error") | No puede existir un `app/(app)/titulo/[slug]` y un `app/(public)/titulo/[slug]` a la vez. Hay exactamente una ruta física; viva en el grupo que viva, la página debe determinar el estado de sesión ella misma (no depender de un guard a nivel de grupo de rutas) para decidir qué rama renderizar. |
| Suposición implícita: el guard de autenticación es `middleware.ts` (tal como lo redacta `ARCHITECTURE.md`) | Esto es Next.js 16. `node_modules/next/dist/docs/01-app/03-api-reference/03-file-conventions/middleware.md`: "The `middleware.js` file convention has been **deprecated** in Next.js 16 and renamed to `proxy.js`." El archivo raíz de guard que RIK-2 debió haber producido es `proxy.ts`, exportando una función `proxy` — no `middleware.ts` | La verificación de AC-1 ("no pasa por el guard de `(app)`") debe hacerse contra el archivo real, sea cual sea el que RIK-2 realmente creó. Si sigue llamándose `middleware.ts` cuando este ticket aterrice, eso es en sí mismo un defecto que este ticket debe señalar, no rodear en silencio, ya que `middleware.ts` no tiene efecto en Next 16 más allá de un shim de deprecación. |
| "Copiar enlace" se lee como ya funcional según la propia descripción de RIK-10 | RIK-10 en sí no está implementado (hermano en paralelo); su texto describe el botón pero no la lógica de generación de URL, y la propia nota de este ticket asigna explícitamente `getPublicListUrl` aquí | Este ticket, no RIK-10, es donde se implementa `getPublicListUrl` y se conecta por primera vez a un `public_code` real. |

### Estado actual en la base de datos

Hoy no existe nada en `supabase/migrations/` (confirmado vacío). Según el propio spec de backlog de RIK-1, `user_lists` se creará así:
...
RLS (RIK-1, de la Sección 9.2 del esquema, aplicada de forma verbatim):
...
**Conclusión clave para este ticket:** este patrón de RLS ya hace el trabajo pesado del AC-3 ("la lista privada deja de resolver"). Cuando `is_public = false`, el `auth.uid()` de una petición anónima es `null`, así que `user_lists_select` evalúa `false` y Postgres devuelve **cero filas** — no hace falta, en rigor, una rama de aplicación tipo "si no es pública y no soy el dueño, bloquear" para hacer cumplir la privacidad, aunque la página igual debe convertir "cero filas" en un `notFound()` (404), no en una página en blanco o rota. `list_items_select` hereda la misma garantía de forma transitiva, así que los títulos de una lista pública también quedan protegidos por RLS sin filtrado adicional.

No existe columna `public_code`, ni tabla de lookup, ni código en el repositorio que genere o resuelva uno — terreno enteramente nuevo dentro de este ticket.

### Lógica actual (lecturas públicas)

No existe implementación actual (proyecto recién scaffoldeado). La referencia más cercana es la intención declarada en `ARCHITECTURE.md`:
...
y la descripción de `features/title` en la tabla de Features:
...
Ambas son directrices a seguir, no código contra el cual diferenciar.

### Mapeo de campos solicitados

| Campo solicitado | Tipo | Equivalente existente | Acción |
| --- | --- | --- | --- |
| Identificador público de lista ("código corto") | string corto único | `user_lists.slug` existe pero es único solo por `(user_id, slug)` — descalificado explícitamente por el propio ticket | **Debe crearse** — nueva columna `user_lists.public_code varchar unique`, poblada al primer publicado |
| Nombre / descripción mostrados públicamente | texto | `user_lists.name`, `user_lists.description` (de RIK-1) | Ya existe (reutilizar) |
| Cuadrícula de títulos (poster, título, año, calificación) | derivado | `list_items` → `media_items` (`title`, `poster_url`, `year`, `imdb_rating`, `slug`, `is_stub`) | Ya existe (reutilizar) |
| `getPublicListUrl` | función | RIK-10 la deja como placeholder con nombre, sin implementación en ningún lado | **Debe crearse** en este ticket |
| Rama pública de `/titulo/[slug]` | ruta/prop | Componente `features/title` (RIK-9, en curso) ya diseñado para una bandera `isPublicView` según `ARCHITECTURE.md` | Extender componente existente; no bifurcar una ruta nueva |
| Layout de la ruta `(public)` | shell de UI | No existe; `ARCHITECTURE.md` reserva "logo + enlaces de auth, sin nav" como la forma prevista | **Debe crearse** — `app/(public)/layout.tsx` |

### Archivos impactados

**migración**
- `supabase/migrations/<timestamp>_user_lists_public_code.sql` (nuevo) — agrega `public_code` a `user_lists`, con timestamp posterior a las dos migraciones de RIK-1.

**types**
- `types/UserList.ts` (o la entrada correspondiente en el barrel `types/index.ts`, según la convención con la que aterricen RIK-1/RIK-10) — agregar `publicCode: string | null`.

**services**
- `services/ListServices/index.ts` — agregar `getPublicListByCode(client, code)` (select acotado, lista + items + media), y generar/persistir `public_code` dentro del método de actualización de visibilidad cuando una lista pasa a pública por primera vez.

**lib**
- `lib/urls.ts` (nuevo) — `getPublicListUrl(publicCode: string | null): string | null`, helper puro de formateo sin acceso a base de datos.

**actions**
- `actions/lists/index.ts` — la acción de cambio de visibilidad llama al método del servicio que asigna `public_code` al primer publicado y lo devuelve en el DTO para que `getPublicListUrl` tenga algo que formatear.

**rutas / app**
- `app/(public)/layout.tsx` (nuevo) — shell mínimo: logo + CTA de login/registro, sin nav, según Sección 2.3 del doc de vistas.
- `app/(public)/l/[codigo]/page.tsx` (nuevo) — Server Component, lee vía `services/ListServices` directamente (sin entrada en `actions/`), llama `notFound()` si la lista falta o es privada.
- `app/(public)/titulo/[slug]/page.tsx` (o donde RIK-9 haya colocado físicamente `/titulo/[slug]` — reubicar si RIK-9 lo anidó dentro de `app/(app)/`, ya que eso lo dejaría inalcanzable sin sesión) — agregar la rama `isPublicView`.
- El archivo raíz `proxy.ts` (nombre en Next 16 de lo que `ARCHITECTURE.md` llama `middleware.ts`) / `lib/supabase/proxy.ts` — verificar que el matcher de pass-through ya cubra las rutas `(public)` y el (posiblemente reubicado) `/titulo/[slug]`; corregir si RIK-2 no anticipó una ruta que vive fuera de `(app)` pero que igual deben poder visitar usuarios autenticados.

**features / components**
- `features/lists/public/` (nuevo) — cuadrícula presentacional para la vista de lista pública.
- `features/title/` — extender con una prop `isPublicView` (componente de RIK-9).
- `components/MediaCard/` — reutilizar si RIK-7/RIK-9 ya lo crearon al momento de implementar; si no, crear una tarjeta local mínima aquí y dejar un TODO de consolidación (no bloquear este ticket por el orden de merge de los hermanos).

**tests**
- No existe suite de pruebas todavía en este proyecto. Anotar dónde deberían vivir las pruebas de componente/ruta cuando se introduzca un runner; no agregar un harness de pruebas como parte de este ticket.

### Decisiones tomadas

1. **`public_code` vive como columna en `user_lists`**, no como tabla de lookup separada — la propia opción sugerida por el ticket, y la más simple dado que RLS ya está acotado por fila en `user_lists`. *Default recomendado, no confirmado por una persona.*
2. **Estrategia de generación:** un string aleatorio corto (10 caracteres) URL-safe (p. ej. derivado de `crypto.randomUUID()` sin guiones y truncado, u otro generador respaldado por CSPRNG equivalente), asignado la primera vez que una lista se marca `is_public = true`, con manejo de colisión reintentando contra la restricción unique de la base (error de Postgres `23505`) en vez de confiar solo en la probabilidad. El código es **estable** a través de sucesivos cambios de público/privado — volver a marcar como pública una lista reutiliza el mismo código, no rota. *Default recomendado.*
3. **Ubicación de `getPublicListUrl`:** `lib/urls.ts`, una función pura que recibe un `publicCode` y devuelve `/l/${publicCode}` (relativo, para que quien la llame controle el origen) o `null` si la lista todavía no tiene código. Usada tanto por el valor de retorno de la acción de cambio de visibilidad como por el botón "copiar enlace" de la UI de RIK-10. *Default recomendado.*
4. **Ubicación física de `/titulo/[slug]`:** debe resolver a una única ruta fuera del guard de autenticación de `(app)` (p. ej. `app/(public)/titulo/[slug]/page.tsx`, o un `app/titulo/[slug]/page.tsx` sin grupo si eso resulta más natural una vez que RIK-9 aterrice) — la página determina el estado de sesión ella misma vía `supabase.auth.getUser()` (sin throw/redirect) y lo pasa hacia abajo como `isPublicView` a `features/title`. Si RIK-9 lo entregó dentro de `app/(app)/titulo/[slug]`, este ticket debe reubicarlo. *Default recomendado / corrección, señalada para verificar contra el resultado real de RIK-9.*
5. **Botones de acción personal para un visitante sin sesión:** por defecto **no renderizar** los controles de "marcar visto" / "agregar a lista" en absoluto para un visitante sin sesión (reemplazados por un CTA de login/registro según la Sección 7.10 del PRD), en vez de renderizar botones inertes que redirigen a login al hacer click. Ambas opciones satisfacen el criterio de aceptación tal como está escrito ("si se muestran, llevan a login"), pero ocultar es más simple y coincide con la propia ficha pública descrita en el PRD. *Default recomendado.*
6. **La reutilización de `MediaCard` es oportunista, no bloqueante.** Si no existe todavía al momento de implementar este ticket, construir una tarjeta local mínima para la cuadrícula pública y señalarla para consolidación futura — no esperar el orden de merge de RIK-7/RIK-9. *Default recomendado.*

### Fuera de alcance

- Descubrimiento entre cuentas / perfiles públicos (explícitamente Fase 3 en el roadmap del PRD, Sección 12).
- Rotar o expirar `public_code` al volver a publicar — el código es estable durante la vida de la lista una vez asignado (ver Decisión 2).
- Rate limiting o protección contra abuso en las rutas públicas — no lo pide ningún criterio de aceptación.
- Generación completa de metadatos SEO/Open Graph para listas compartidas — solo el comportamiento `noindex` que `notFound()` ya da gratis está en alcance; un `generateMetadata`/generación de imagen OG dedicados quedan para después.
- Construir `user_lists`/`list_items`, la lógica central del guard de `(app)`, o la rama autenticada de `features/title` desde cero — eso es responsabilidad de RIK-1, RIK-2 y RIK-9 respectivamente; este ticket solo los extiende/reubica/verifica donde la rama pública lo requiere.

---

## Plan de implementación

**Objetivo:** conectar de punta a punta la única superficie sin sesión deliberada de Rikuna — una lista pública y sus títulos — desde una nueva columna `public_code` hasta dos páginas reales, sin que una petición anónima toque nunca nada más de la cuenta del dueño.

**En alcance:**
1. Migración nueva: `user_lists.public_code varchar unique`, más un índice de búsqueda, agregada sobre el esquema de RIK-1 (nunca editando el archivo de RIK-1).
2. `types/UserList` gana `publicCode`.
3. `services/ListServices`: `getPublicListByCode()` (lectura acotada: solo lista + items + media, lista explícita de columnas, sin datos de dueño en la forma devuelta) y asignación de código público dentro del flujo existente de actualización de visibilidad.
4. `lib/urls.ts`: `getPublicListUrl()`.
5. `actions/lists`: conectar el toggle de visibilidad para persistir/devolver `public_code`.
6. `app/(public)/layout.tsx`: shell mínimo (logo + CTA de login/registro, sin nav).
7. `app/(public)/l/[codigo]/page.tsx`: Server Component, `notFound()` si la lista falta o es privada.
8. `/titulo/[slug]`: confirmar o reubicar a una ubicación alcanzable sin sesión; agregar el control `isPublicView` en `features/title`.
9. Verificar que el `proxy.ts` raíz / `lib/supabase/proxy.ts` realmente dejen pasar las rutas `(public)` y `/titulo/[slug]`.

**Fuera de alcance:** ver arriba — descubrimiento, rotación de código, rate limiting, metadatos OG completos, y todo lo que pertenece al propio alcance de RIK-1/RIK-2/RIK-9.

**Riesgos clave / compatibilidad:**
- El mayor riesgo es la filtración accidental de datos: cualquier consulta en la ruta pública que no esté acotada exactamente a las filas de la lista/media solicitadas (por ejemplo, unir `user_subscriptions` o `user_media_status`, o hacer `select *` sobre `user_lists` y reenviarlo entero al cliente) violaría la regla de privacidad de la Sección 11 del PRD. La selección explícita de columnas en `getPublicListByCode` es obligatoria, no basta con confiar solo en RLS.
- Los grupos de rutas de Next.js no pueden tener dos carpetas que resuelvan a la misma URL — esto condiciona dónde puede vivir físicamente `/titulo/[slug]`; equivocarse aquí es un error en tiempo de build, no un bug sutil, pero solo una vez que alguien intente tener ambos.
- Nombre `proxy.ts` vs `middleware.ts`: si RIK-2 usó el nombre deprecado, este ticket debe señalarlo en vez de rodearlo en silencio.

**Mapeo de criterios de aceptación:**

| AC | Satisfecho por |
| --- | --- |
| AC-1 | `app/(public)/l/[codigo]/page.tsx` fuera de `(app)`; pass-through verificado en `proxy.ts` |
| AC-2 | Selección explícita de columnas en `getPublicListByCode`; ninguna otra llamada a servicios desde la página pública |
| AC-3 | Comportamiento de cero filas de RLS cuando `is_public = false` + `notFound()` cuando el servicio no devuelve nada |
| AC-4 | Rama `isPublicView` en `features/title`; ningún botón con guard de sesión ejecuta acción |
| AC-5 | Nueva columna `public_code` + restricción unique + generación con reintento en colisión, independiente de `slug` |
