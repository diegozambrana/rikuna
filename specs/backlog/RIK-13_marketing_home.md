...
Replace the `create-next-app` placeholder at `/` with the real Rikuna marketing landing page: a Hero (name, tagline, two CTAs), a "Cómo funciona" 4-step section, a trust section explaining the IMDb-driven approach, and a minimal footer — plus a session-aware redirect so a logged-in visitor lands on `/panel` instead of seeing the marketing page at all.

- `app/page.tsx` today is 100% unmodified `create-next-app` boilerplate (Next.js/Vercel logos, "To get started, edit page.tsx" copy) — this ticket is a full replacement, not an edit.
- Moves the route into a new `(marketing)` route group per `ARCHITECTURE.md`'s already-documented target structure, with its own layout rendering the Header/Sidebar **guest variant** built in RIK-12 — this ticket `depends_on` RIK-12 for those components to exist.
- Content is grounded in `specs/RIKUNA-PRD-documento-especificacion-rikuna.md` (Sections 1 and 5): the quechua-origin tagline, the three-set-crossing value proposition, and the "starts from your real IMDb history" trust angle — not invented copy.
- No team comments — derived from the same gap-analysis pass as RIK-12 (see RIK-12's Original ticket for how the four-ticket series was scoped). Font family / typography is explicitly out of scope per the requester.
...
No tracker ticket exists for this work; scoped from a direct comparison between `specs/RIKUNA-PRD-vistas-y-estilo-rikuna-v2.md` Section 2.0 ("Zona Marketing — `/` Inicio") and the real `app/page.tsx`, which is still the unmodified Next.js starter page.

PRD requirements folded in here (Section 2.0, verbatim intent):

- **Purpose:** entry page for anyone arriving without a session (today this gap simply didn't exist). If the visitor already has a session, redirect straight to `/panel` instead of showing this page.
- **Hero:** "Rikuna" name + meaning ("lo que se debe ver", Quechua), one-line value proposition, two buttons ("Crear cuenta", "Iniciar sesión").
- **Cómo funciona (3-4 steps):** import your IMDb history → declare your active service → get your monthly list → discover something new, well-rated.
- **Trust section:** brief note that the data starts from the user's real IMDb ratings, not a generic algorithm.
- **Simple footer:** link to login/register only — no legal/corporate content defined yet.
- **Layout:** Marketing (Header + Sidebar, guest variant, per Section 1.6 — built in RIK-12, reused here).
- **Suggested components:** stacked sections with `Card` or simple blocks, `Button` for CTAs, Lucide icons per step.
...
| --- | --- | --- |
| PRD describes `/` as a gap to fill | `app/page.tsx` exists but is the exact, untouched `create-next-app` starter (Next.js logo, Vercel deploy CTA, "Learn" links) — confirmed by reading the file in full | This is a full replacement of the file's contents, not an incremental edit; nothing in the current file is reused |
| PRD Section 2.2 lists `/panel` as "ya no es `/`" (implying `/` used to be the landing page) | `app/(app)/panel/page.tsx` already exists as its own route, unaffected by this ticket — `/` and `/panel` have always been separate physical routes in this codebase, there is no legacy `/` behavior to migrate away from | No migration work needed; this is additive |
| `ARCHITECTURE.md` (uncommitted, modified) already documents a `(marketing)` route group: `app/(marketing)/page.tsx` → `/`, "Redirects to `/panel` if a session already exists" | No `(marketing)` route group exists yet; `app/page.tsx` sits directly under `app/` with no route group, inheriting only the root layout | This ticket creates `app/(marketing)/` and moves the file into it — Next.js route groups don't affect the URL, so `/` stays `/` |
| PRD implies the session-redirect happens "automatically" | `lib/supabase/proxy.ts`'s `AUTH_ONLY_PATHS` (middleware-level redirect for authenticated users) currently only lists `/auth/login` and `/auth/sign-up` — `/` is not in that list, so today an authenticated visitor to `/` would see the (currently broken) marketing page, not get redirected | This ticket must add the redirect logic itself — either by extending `AUTH_ONLY_PATHS` to include `/`, by a Server Component check in the new `(marketing)/layout.tsx`, or both (see Decisions) |
| PRD Section 1.6 describes the Marketing Header/Sidebar as if independent components | `ARCHITECTURE.md`'s Shared UI section states: "Both [Header and Sidebar] render in `(marketing)` and `(app)`" — i.e. the SAME components, not a parallel set | This ticket must reuse `components/layout/Header.tsx` / `Sidebar.tsx` from RIK-12, extending their prop types for the guest case rather than building new components |

### Current database state

Not applicable — this ticket is purely presentational; it reads session state (via `getCurrentUser()`, already implemented) but writes nothing and touches no tables.

### Current logic (`app/page.tsx`)

Verbatim (abridged), the entire file today:
...
It performs no session check, has no Header/Sidebar (it is not inside any route group), and references `public/next.svg` and `public/vercel.svg` — the only two files in the whole repo that import those assets (confirmed via `grep`). Nothing in this file's structure, styling, or logic carries forward.

### Requested field mapping

Not applicable — no persisted fields. This ticket's only data dependency is `getCurrentUser(): Promise<CurrentUser | null>` (`lib/supabase/server.ts`), already implemented and reused as-is.

### Impacted files

**App routes**
- `app/(marketing)/page.tsx` — new. The real Hero/Cómo funciona/trust/footer content, replacing `app/page.tsx`.
- `app/(marketing)/layout.tsx` — new. Session check (`getCurrentUser()`), `redirect("/panel")` if present, otherwise renders `Header`/`Sidebar` (guest variant) + `{children}`.
- `app/page.tsx` — **deleted**. Its content moves to `app/(marketing)/page.tsx`; Next.js resolves both to the same `/` URL, and having both simultaneously would be a route collision.

**Components (modified, from RIK-12)**
- `components/layout/Header.tsx` — extend the `user` prop type from `CurrentUser` to `CurrentUser | null`; when `null`, render "Iniciar sesión"/"Crear cuenta" buttons instead of the avatar/dropdown, per PRD Section 1.6's "Sin sesión" column.
- `components/layout/Sidebar.tsx` — no type change needed; reused as-is with a different `items` array (see below). Its active-route highlighting is a no-op for anchor-fragment items, which is an accepted simplification (see Decisions).

**Constants (new)**
- `constants/marketingNavigation.ts` — the guest-variant nav item list: "Inicio" (`#inicio`), "Cómo funciona" (`#como-funciona`), "Iniciar sesión" (`/auth/login`), "Crear cuenta" (`/auth/sign-up`) — mirrors `constants/navigation.ts`'s `NavItem` shape from RIK-12 so `Sidebar`/`SidebarNavItem` need no further changes.

**Features (new)**
- `features/marketing/Hero.tsx` — name, tagline, two CTA buttons.
- `features/marketing/HowItWorks.tsx` — 4-step list with Lucide icons, anchored at `#como-funciona`.
- `features/marketing/TrustSection.tsx` — short IMDb-sourced-data trust note.
- `features/marketing/MarketingFooter.tsx` — minimal footer, login/register links only.

**Config (modified)**
- `lib/supabase/proxy.ts` — add `"/"` to `AUTH_ONLY_PATHS` so an authenticated visitor is redirected at the middleware layer, not just after a full render round-trip (see Decisions).

**No changes** to `services/`, `actions/`, `types/`, or any `supabase/migrations/` file. `public/next.svg` / `public/vercel.svg` become unused but are left in place (harmless; not part of this ticket's scope to prune starter assets).

### Decisions made

1. **`AUTH_ONLY_PATHS` gains `"/"` (middleware-level redirect) AND `(marketing)/layout.tsx` keeps its own `getCurrentUser()` check (Server Component belt-and-suspenders).** Rationale: mirrors the exact pattern `app/(app)/layout.tsx`'s own comment already documents for the opposite direction — "Server Functions bypass proxy matchers, so this Server Component check is the real backstop." The middleware redirect is the fast path; the layout check is correctness under all circumstances. Recommended default, unconfirmed — flagged in `<clarify_before_coding>` since it touches a shared, already-correct file.
2. **`Header`'s `user` prop becomes `CurrentUser | null`, extending RIK-12's component rather than forking a second one.** Rationale: `ARCHITECTURE.md` explicitly states Header/Sidebar render in both `(marketing)` and `(app)` — a parallel `MarketingHeader` would contradict that and double the maintenance surface for one logo + one link cluster's worth of difference. Recommended default.
3. **Sidebar's guest item list uses in-page anchor hrefs (`#inicio`, `#como-funciona`) for the first two items, and real routes for the last two ("Iniciar sesión" → `/auth/login`, "Crear cuenta" → `/auth/sign-up`).** Rationale: matches PRD Section 1.6's guest sidebar table exactly ("Inicio: Ancla al tope de la propia página"; "Cómo funciona: Ancla a la sección correspondiente"). Active-route highlighting via `usePathname()` (built in RIK-12) is meaningless for anchor items and is accepted as a known no-op for those two rows — not worth special-casing `SidebarNavItem` for a page that is always `/`. Recommended default.
4. **No new UI primitives needed.** All of `Card`, `Button`, and the Lucide icon set are already available (`Button` since project inception; `Card` added in RIK-9). Confirmed via `ls components/ui/`.
5. **Copy is written from `specs/RIKUNA-PRD-documento-especificacion-rikuna.md` Sections 1 and 5**, not invented: the Quechua meaning ("lo que se debe ver"), the core question framing ("este mes contraté tal servicio de streaming, ¿qué de todo lo que quiero ver puedo ver ahora?"), and the three differentiators (real IMDb history, never suggests repeats, adapts to the currently active service). Confirmed by reading the source document.

### Out of scope

- Any change to `components/layout/Sidebar.tsx`'s internal logic — reused verbatim from RIK-12 with a different `items` prop value.
- Legal/corporate footer content — PRD explicitly says "nada más — no hay contenido legal/corporativo definido todavía."
- Pruning the now-unused `public/next.svg` / `public/vercel.svg` starter assets — harmless leftovers, not required.
- Brand accent color — PRD Section 1.4 marks this "Pendiente de definir"; this ticket uses the existing `mist`-derived neutral palette and default `Button`/`Card` variants only.
- Font family — explicitly excluded from this whole gap-analysis pass by the requester.

---

## Implementation plan

**Goal:** Give Rikuna a real, session-aware `/` landing page — closing the second PRD gap in this series — built entirely on top of RIK-12's Header/Sidebar rather than duplicating navigation chrome.

**In scope:**
1. `lib/supabase/proxy.ts` — add `"/"` to `AUTH_ONLY_PATHS`.
2. `components/layout/Header.tsx` — extend `user` prop to `CurrentUser | null`; guest branch renders "Iniciar sesión"/"Crear cuenta" buttons.
3. `constants/marketingNavigation.ts` — guest nav item list.
4. `features/marketing/{Hero,HowItWorks,TrustSection,MarketingFooter}.tsx`.
5. `app/(marketing)/layout.tsx` — session check + redirect + Header/Sidebar (guest) composition.
6. `app/(marketing)/page.tsx` — composes the four feature sections.
7. Delete `app/page.tsx`.

**Out of scope:** Sidebar internal changes, legal footer content, unused starter-asset cleanup, brand accent color, font family — see Out of scope above.

**Key risks / compatibility:**
- Deleting `app/page.tsx` while adding `app/(marketing)/page.tsx` must happen atomically — having both simultaneously is a Next.js route collision on `/`.
- Extending `Header`'s prop type must not break its existing `(app)` call site (RIK-12's `app/(app)/layout.tsx` passes a non-null `CurrentUser`) — the change is additive (widen the type, add a branch), not a breaking rename.
- `constants/marketingNavigation.ts` must satisfy the exact same `NavItem` type RIK-12 defined in `constants/navigation.ts`, or `Sidebar`/`SidebarNavItem` will need type changes this ticket doesn't intend to make.

**Acceptance criteria mapping:**

| AC | Satisfied by |
| --- | --- |
| AC-1 | `app/(marketing)/layout.tsx` session check + redirect |
| AC-2 | `Hero.tsx` — name, tagline, two CTAs |
| AC-3 | `HowItWorks.tsx` — 4 steps with icons |
| AC-4 | `TrustSection.tsx` |
| AC-5 | `MarketingFooter.tsx` |
| AC-6 | `Header.tsx` guest branch + `Sidebar` with `constants/marketingNavigation.ts` |
| AC-7 | `app/page.tsx` deletion + `app/(marketing)/page.tsx` creation, single route |
